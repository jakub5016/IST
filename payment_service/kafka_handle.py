                ...
                if not appointment_id:
                    ...
                    if payment:
                        if payment.get("refunded") is True:
                            logger.info(f"Refund already processed for appointment_id: {appointment_id}, skipping refund message.")
                            continue
                        if payment.get("paymentMethod") == "payu":
                            orderData = {"uuid": appointment_id}
                            send_message(orderData, REFUND_REQUESTED_TOPIC)
                            logger.info(f"Refund requested for payu payment: {orderData}")
                        else:
                            logger.info(f"No refund required for non-payu payment method: {payment.get('paymentMethod')}")
                    else:
                        logger.warning(f"No payment found for cancelled appointment_id: {appointment_id}")

            elif topic == NEW_APPOINTMENT_TOPIC:
                logger.info("New appointment appeared")

                existing = PAYMENTS_COLLECTION.find_one({"uuid": value['appointmentId']})
                if existing:
                    logger.warning(f"Payment with uuid {value['appointmentId']} already exists. Skipping insertion.")
                    continue
